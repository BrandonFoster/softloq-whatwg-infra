...
template <class T> infra_sequence<T>::infra_sequence(const infra_sequence& src) noexcept : infra_sequence(src, std::lock_guard<std::mutex>(src.main_mtx)) {}
template <class T> infra_sequence<T>::infra_sequence(infra_sequence&& src) noexcept : infra_sequence(std::move(src), std::lock_guard<std::mutex>(src.main_mtx)) {}
...
template <class T> infra_sequence<T>& infra_sequence<T>::operator+=(const infra_sequence& sequence) noexcept
{
    #ifdef SOFTLOQ_MULTITHREADING
    std::lock_guard<std::mutex> lock(main_mtx);
    std::lock_guard<std::mutex> sequence_lock(sequence.main_mtx);
    #endif
    for (const auto& item: sequence.values) values.push_back(item);
    return *this;
}
...
template <class T> const bool infra_sequence<T>::operator==(const infra_sequence<T>& b) const noexcept
{
    #ifdef SOFTLOQ_MULTITHREADING
    std::lock_guard<std::mutex> lock(main_mtx);
    std::lock_guard<std::mutex> b_lock(b.main_mtx);
    #endif
