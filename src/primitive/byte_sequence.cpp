...
SOFTLOQ_WHATWG_INFRA_API infra_byte_sequence::infra_byte_sequence(const std::initializer_list<infra_byte>& values) noexcept : infra_sequence<infra_byte>(values) {}
SOFTLOQ_WHATWG_INFRA_API infra_byte_sequence::infra_byte_sequence(const std::string& values) noexcept { std::transform(values.cbegin(), values.cend(), std::back_inserter(*this), [](const char c) { return infra_byte{c}; }); }
SOFTLOQ_WHATWG_INFRA_API infra_byte_sequence::infra_byte_sequence(const std::u8string& values) noexcept { std::transform(values.cbegin(), values.cend(), std::back_inserter(*this), [](const std::uint8_t c) { return infra_byte{c}; }); }
SOFTLOQ_WHATWG_INFRA_API infra_byte_sequence::infra_byte_sequence(const infra_byte_sequence& src) noexcept : infra_sequence<infra_byte>(*static_cast<const sequence_type*>(this)) {}
SOFTLOQ_WHATWG_INFRA_API infra_byte_sequence::infra_byte_sequence(infra_byte_sequence&& src) noexcept : infra_sequence<infra_byte>(std::move(*static_cast<sequence_type*>(this))) {}
SOFTLOQ_WHATWG_INFRA_API infra_byte_sequence::~infra_byte_sequence() noexcept {}
...
SOFTLOQ_WHATWG_INFRA_API const bool infra_byte_sequence::operator>(const infra_byte_sequence& b) const noexcept { return is_byte_less(b, *this); }
SOFTLOQ_WHATWG_INFRA_API const bool infra_byte_sequence::operator==(const infra_byte_sequence& b) const noexcept { return *static_cast<const sequence_type*>(this) == *static_cast<const sequence_type*>(&b); }
